...
impl H7CAD {
    ...
    fn pid_locate_handles(&mut self, i: usize, handles: &[Handle]) {
        ...
        let wanted: std::collections::HashSet<String> = handles
            .iter()
    ...
    pub fn update(&mut self, msg: Message) -> Task<Message> {
        match msg {
            ...
            Message::FileOpened(Ok(open_result)) => {
                ...
                let open_message = match opened {
                    ...
                    crate::io::OpenedDocument::Pid(bundle) => {
                        ...
                        let compat_preview = crate::io::native_bridge::native_doc_to_acadrust(
                            &bundle.native_preview,
                        );
                        ...
                        self.tabs[i]
                            .scene
                            .set_native_doc(Some(bundle.native_preview));
                        self.tabs[i].scene.native_render_enabled =
                            self.tabs[i].native_render_enabled;
            ...
            Message::WorkspaceOpen => {
                Task::perform(crate::io::pick_workspace_folder(), Message::WorkspaceOpened)
            }
            ...
            Message::WorkspaceToggle => {
                if self.workspace.is_none() {
                    self.command_line.push_info(
                        "WORKSPACE: no workspace is open — use WORKSPACE to pick a folder first.",
                    );
                } else {
            ...
            Message::WorkspaceFileClick(path) => {
                ...
                let existing = self
                    .tabs
                    .iter()
                    .position(|t| t.current_path.as_deref() == Some(path.as_path()));
            ...
            Message::CuiExport => {
                Task::perform(crate::io::pick_cui_save_path(), Message::CuiExportPath)
            }
            ...
            Message::CuiImport => {
                Task::perform(crate::io::pick_cui_open_path(), Message::CuiImportPath)
            }
            ...
            Message::CuiLoad => {
                Task::perform(crate::io::pick_cui_open_path(), Message::CuiLoadPath)
            }
            Message::CuiLoadPath(Some(path)) => {
                    ...
                    .and_then(|s| crate::io::cui::parse_cui(&s))
                {
                    Ok(doc) => {
                        ...
                        self.command_line
                            .push_output(&format!("CUILOAD: merged {}", path.display()));
                        self.command_line.push_info(&format!(
            ...
            Message::RibbonToolClick { tool_id, event } => {
                ...
                match event {
                    ...
                    ModuleEvent::OpenFileDialog => {
                        return Task::perform(crate::io::pick_and_open(), Message::FileOpened);
                    }
            ...
            Message::OsWindowClosed(id) => {
                ...
                if self.svg_export_window == Some(id) {
                    self.svg_export_window = None;
                }
                if self.pdf_export_window == Some(id) {
                    self.pdf_export_window = None;
                }
                ...
                if is_down && is_click && self.tabs[i].active_cmd.is_some() && !is_gathering {
                        ...
                        .unwrap_or(false)
                    {
                        ...
                        let hit = scene::hit_test::click_hit(p, &all_wires2[..], vp_mat2, bounds)
                            .and_then(|s| Scene::handle_from_wire_name(s))
                            .or_else(|| {
                                scene::hit_test::click_hit_hatch_entries(
                                    p,
                                    &hatch_entries,
                                    vp_mat2,
                                    bounds,
                        ...
                        if let Some(handle) = hit {
                            ...
                            {
                                if let Some(model) =
                                    self.tabs[i].scene.hatch_model_for_handle(handle)
                                {
                                    use crate::command::CadCommand;
            ...
            Message::PropBoolToggle(field) => {
                ...
                let summary = self.apply_store_edit(i, "CHPROP", |store, handle| {
                    ...
                    match field {
                        ...
                        _ => {
                            crate::scene::dispatch::apply_geom_prop_native(entity, field, "toggle")
                        }
                    }
                });
                self.finish_property_edit(i, summary);
            ...
            Message::SvgExport(opts) => {
                ...
                let stem = self.tabs[i]
                    ...
                    .map(|s: &std::ffi::OsStr| s.to_string_lossy().into_owned())
                    .unwrap_or_else(|| "drawing".into());
                Task::perform(crate::io::svg_export::pick_svg_path_owned(stem), move |p| {
                    Message::SvgExportPath(p, opts.clone())
                })
            }
            ...
            Message::SvgExportPath(Some(path), opts) => {
                ...
                match crate::io::svg_export::export_svg_full(
                    ...
                    Err(e) => self
                        .command_line
                        .push_error(&format!("SVG export failed: {e}")),
                }
                Task::none()
            }
            ...
            Message::PdfExportDialogCommit => {
                ...
                let pick_task =
                    Task::perform(crate::io::pdf_export::pick_pdf_path_owned(stem), move |p| {
                        Message::PlotExportPathWithOpts(p, opts_for_picker.clone())
                    });
...
mod tests {
    ...
    fn build_fixture_pid(path: &Path) {
        ...
        let drawing =
            b"<?xml version=\"1.0\"?><Drawing><Tag SP_DRAWINGNUMBER=\"FX-OPEN\"/></Drawing>";
    ...
    fn prop_geom_commit_updates_native_line_when_compat_missing() {
        ...
        assert!(
            app.tabs[0].dirty,
            "native geometry commit should mark the tab dirty"
        );
    }
    ...
    fn file_opened_pid_bundle_populates_pid_tab_state_and_preview_scene() {
        ...
        assert!(
            app.tabs[0].is_pid(),
            "pid file should switch the active tab into pid mode"
        );
    ...
    fn prop_layer_changed_updates_native_text_when_compat_missing() {
        ...
        assert!(
            app.tabs[0].dirty,
            "native common property change should mark the tab dirty"
        );
    }
    ...
    fn viewport_move_updates_native_line_grip_when_compat_missing() {
        ...
        assert!(
            app.tabs[0].dirty,
            "native grip move should mark the tab dirty"
        );
    }
    ...
    fn prop_geom_commit_updates_native_dimension_when_compat_missing() {
        ...
        assert!(
            app.tabs[0].dirty,
            "native dimension geom edit should mark the tab dirty"
        );
    }
