...
use iced::widget::{
    button, canvas, column, container, mouse_area, row, shader, stack, text, Row, Space,
};
use iced::window;
    ...
    pub fn view(&self, window_id: window::Id) -> Element<'_, Message> {
        ...
        let center_row: Element<'_, Message> = if tab.is_pid() {
            let pid_state = tab
                .pid_state
                .as_ref()
                .expect("pid tab should carry pid_state");
            ...
            let toolbar = container(
                row![
                    ...
                    button(
                        text(if pid_state.hide_unresolved {
                            "Show Unresolved"
                        } else {
                            "Hide Unresolved"
                        })
            ...
            let graph_column: Element<'_, Message> = column![toolbar, viewport_stack]
                .width(Fill)
                .height(Fill)
                .into();
            let properties_el: Element<'_, Message> = if self.show_properties {
